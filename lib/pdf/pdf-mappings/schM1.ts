...
export const SCH_M1_MAPPING: FormPdfMapping = {
  ...
  fields: [
    ...
    },
    {
      ...
      compute: (ctx: FillContext) => { const cg = Number(ctx.facts.capital_gain_total ?? 0); return cg < 0 ? String(Math.abs(cg)) : "0"; },
      format: "currency",
      irsLine: "3",
    ...
    // -------------------------------------------------------------------------
    {
      ...
      factName: "tax_exempt_interest_total",
      format: "currency",
      irsLine: "7a",
    ...
    },
    {
      ...
      compute: (ctx: FillContext) => String(Number(ctx.facts.tax_exempt_interest_total ?? 0)),
      format: "currency",
      irsLine: "7c",
    ...
    },
    {
      ...
      compute: (ctx: FillContext) => { const ni = Number(ctx.facts.net_income_before_tax ?? 0); const tax = Number(ctx.facts.income_tax_expense_total ?? 0); const ce = Number(ctx.facts.charitable_contributions_excess ?? 0); const meals = Number(ctx.facts.m1_meals_disallowance ?? 0); const nd = Number(ctx.facts.nondeductible_total ?? 0); const tei = Number(ctx.facts.tax_exempt_interest_total ?? 0); return String(ni + tax + ce + meals + nd - tei); },
      format: "currency",
      irsLine: "10",
